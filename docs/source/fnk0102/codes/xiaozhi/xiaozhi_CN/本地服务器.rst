...
如果您还没有安装Ollama，请访问 https://ollama.com/download 进行下载和安装。
...
请访问 https://ollama.com/search，选择适合您电脑的，或者您喜欢的LLM模型。
...
选择适合您电脑平台的软件包下载。Miniconda 是 Anaconda 提供的一个轻量级安装程序，已预先配置好，可直接用于 Anaconda 软件仓库。
...
:combo:`red font-bolder:请注意，有时候使用开启虚拟环境会提示您需要使用“conda init”指令。请执行它，并重启终端。`
...
.. note:: 
    ...
    :combo:`red font-bolder:如果您的Vscode没有正确安装，使用指令可能会报错。您同样可以手动使用Vscode打开这个文件。`
...
:combo:`red font-bolder:请注意：为确保您能够顺利使用小智AI的视觉识别功能，请按照以下步骤配置视觉识别大模型（VLLM）；若您当前无需使用该功能，则可直接跳过此步骤继续后续操作。`

继续修改config.yaml文件，先按照提示步骤注册对应的api key，再将生成的api key填入到代码中

.. image:: ../../_static/imgs/xiaozhi/Local_Server/Chapter03_105.png
    :align: center
